...
/*   Updated: 2019/12/29 11:51:47 by ikhadem          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cast_floor(t_game *g)
{
	...
	for (int y = g->cam.drawend + 1; y < g_win.resolution.y; y++)
	{
		g->dfc.currentdist = g_win.resolution.y / (2.0 * y - g_win.resolution.y);
		...
		int pos = g->dfc.texx * 4 + g->dfc.texy * 4 * g->t[4].h;
		...
		pos2 = g->cam.id * 4 + g_win.resolution.x * 4 * (g_win.resolution.y - y);
